...
`main` 分支不再接受 Pull Request。新功能开发应在独立分支或 fork 中维护。维护者会在符合当前维护范围时，选择性地将小型、经过审查的修复整理回发布分支。

1. Fork 仓库
2. 为你的工作创建聚焦的独立分支
3. 只修改当前 bug 修复、文档更新或平台体验优化所需的范围
4. 分享补丁或分支前运行下面的检查命令

### 重要说明

除非某次维护目标明确要求，否则补丁不得增加无关功能，也不得改变现有 Windows 行为。macOS/Linux 体验优化应保持在平台专属路径内，不能回归 Windows 发布链路。

所有改动都必须遵守 [Developer Code Standards](docs/developer_code_standards.md)。提交审阅前请运行：
