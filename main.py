...
def _ensure_gui_deps_or_prompt(pyexe: str | None):
    ...
    msg = (
        "检测到缺少 GUI 依赖：PyQt6 / PyQt6-Fluent-Widgets。\n\n"
        f"是否现在安装？\n\n将执行：\n{shown_cmd}"
    )
...
if _is_packaged_mode():
    ...
    if py_exe.exists():
        ...
        if os.environ.get("LATEXSNIPPER_INNER_PY") != "1":
            print(f"[INFO] 打包模式：重定向到私有解释器 {py_exe}")
            import subprocess
            env = os.environ.copy()
            ...
            print(f"[INFO] 打包模式：已在私有解释器环境，继续执行主程序")
    else:
        print(f"[ERROR] 打包模式但找不到私有解释器: {py_exe}")
        sys.exit(8)
...
if os.environ.get("LATEXSNIPPER_BOOTSTRAPPED") != "1":
    ...
    # 5) 现在调用层级向导 UI（仅此一次；显式展示）
    # 传入已选择的 BASE_DIR，避免重复弹窗
...
from PyQt6.QtWidgets import QVBoxLayout, QListWidget, QMenu, QInputDialog
...
<script>
  window.MathJax = {
    ...
    options: {
      enableMenu: false,
      skipHtmlTags: [],  # 清空跳过标签列表，确保处理所有内容
      ignoreHtmlClass: [],  # 不忽略任何类
      processHtmlClass: []  # 移除类限制，处理所有 HTML 内容
    }
  };
</script>
</head>
<body>
__FORMULAS__
<!-- MathJax 脚本会由 setHtml 的 base URL 动态加载 -->
...
def _get_mathjax_base_url():
    ...
    try:
        ...
        # 构建 es5 目录路径
        es5_dir = base / "assets" / "MathJax-3.2.2" / "es5"
        tex_chtml = es5_dir / "tex-mml-chtml.js"
        
        print(f"[DEBUG _get_mathjax_base_url] base={base}")
        print(f"[DEBUG _get_mathjax_base_url] es5_dir={es5_dir}")
        print(f"[DEBUG _get_mathjax_base_url] exists={tex_chtml.exists()}")
        ...
        # 转换为 file:// URL，确保路径末尾有 /
        url = QUrl.fromLocalFile(str(es5_dir) + "/")
        url_str = url.toString()
        print(f"[DEBUG _get_mathjax_base_url] final URL={url_str}")
        return url
...
def latex_to_svg(latex: str) -> str:
    ...
    try:
        ...
        import html
        # 降级处理：返回纯文本
        return f'<div style="color:red;font-size:12px;padding:8px;background:#ffebee;border-radius:4px;">❌ 转换失败: {html.escape(str(e)[:50])}</div>'
...
class FavoritesWindow(_QMainWindow):
    ...
    def _latex_to_mathml(self, latex: str) -> str:
        ...
        try:
            ...
            self._set_status(f"MathML 转换失败: {e}")
            escaped = latex.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            return _mathml_standardize(f'''<math xmlns="http://www.w3.org/1998/Math/MathML">
  <mrow>
    <mtext>{escaped}</mtext>
  </mrow>
</math>''')
    ...
    def _latex_to_omml(self, latex: str) -> str:
        ...
        try:
            ...
            self._set_status(f"OMML 转换失败: {e}")
            return ""
    ...
    def add_favorite(self, text: str, content_type: str = None, name: str = None):
        ...
        if content_type is None:
            p = self.parent()
            if p and hasattr(p, "current_model"):
...
class SettingsWindow(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self._model_options = [
            ...
            ("pix2text", "pix2text - 公式识别 (精准)"),
            ("pix2text_mixed", "pix2text - 混合识别 (文字+公式)"),
            ("pix2text_text", "pix2text - 纯文字识别"),
            ("pix2text_page", "pix2text - 整页识别 (含布局)"),
        ...
        self.btn_terminal.setToolTip("在当前 Python 环境中打开命令行终端，可手动安装/修复依赖。\n⚠️ 请谨慎操作，错误的命令可能损坏环境！")
        lay.addWidget(self.btn_terminal)
    ...
    def _update_model_desc(self):
        ...
        descriptions = {
            "pix2tex": "💡 轻量级公式识别，速度快，适合简单公式。",
            "pix2text": "🎯 高精度公式识别，支持复杂公式，需要 GPU。",
            "pix2text_mixed": "📝 混合识别文字和公式，自动区分内容类型。",
            "pix2text_text": "📖 纯文字识别，用于截取普通文本。",
            "pix2text_page": "📄 整页识别，包含布局分析，适合文档截图。",
            "pix2text_table": "📊 表格识别，输出 HTML/Markdown 格式表格。",
        }
        desc = descriptions.get(key, "")

        # 检查是否需要 GPU
        if key.startswith("pix2text"):
            desc += "\n⚠️ 需要安装 PIX2TEXT_GPU 层"
    ...
    def _open_terminal(self):
        ...
        msg = MessageBox(
            "打开终端",
            "是否以管理员身份打开终端？\n\n"
            "• 管理员模式：可解决权限问题（推荐用于修复依赖）\n"
            "• 普通模式：快速打开，但可能遇到权限错误\n"
            "• ESC取消操作",
            self
        )
        msg.yesButton.setText("管理员模式")
        msg.cancelButton.setText("普通模式")
        ...
        pyexe = os.environ.get("LATEXSNIPPER_PYEXE", sys.executable)
        # 获取 Python 环境目录
        pyexe_dir = os.path.dirname(pyexe)
        scripts_dir = os.path.join(pyexe_dir, "Scripts")

        # 检查 Scripts/activate.bat 是否存在（venv/virtualenv 环境）
        activate_script = os.path.join(scripts_dir, "activate.bat")
        has_activate = os.path.exists(activate_script)

        # 如果没有 activate.bat，检查是否有 pip.exe（独立 Python 安装）
        has_scripts = os.path.exists(os.path.join(scripts_dir, "pip.exe"))

        venv_dir = pyexe_dir  # 工作目录设为 Python 环境目录

        # 常用修复命令提示
        help_cmds = [
            "echo.",
            "echo ================================================================================",
            "echo                        LaTeXSnipper 环境终端 - 常用命令",
            "echo ================================================================================",
            "echo.",
            "echo [版本修复 - 解决常见依赖冲突]",
            "echo   pip install numpy==1.26.4                    # 修复 numpy (必须 ^<2.0)",
            "echo   pip install protobuf==4.25.8                 # 修复 protobuf (必须 ^<5.0)",
            "echo   pip install pydantic==2.9.2 pydantic-core==2.23.4  # 修复 pydantic 版本冲突",
            "echo.",
            "echo [PyTorch GPU 版本 - 根据你的 CUDA 版本选择]",
            "echo   nvcc --version                               # 查看你的 CUDA Toolkit 版本",
            "echo   # CUDA 11.8:",
            "echo   pip install torch==2.7.1 torchvision==0.22.1 --index-url https://download.pytorch.org/whl/cu118",
            "echo   # CUDA 12.1:",
            "echo   pip install torch==2.7.1 torchvision==0.22.1 --index-url https://download.pytorch.org/whl/cu121",
            "echo   # CUDA 12.4:",
            "echo   pip install torch==2.7.1 torchvision==0.22.1 --index-url https://download.pytorch.org/whl/cu124",
            "echo   # CPU 版本:",
            "echo   pip install torch==2.7.1 torchvision==0.22.1 --index-url https://download.pytorch.org/whl/cpu",
            "echo.",
            "echo [ONNX Runtime - GPU 加速]",
            "echo   pip install onnxruntime-gpu==1.18.1          # CUDA 11.8 (需匹配 PyTorch)",
            "echo   pip install onnxruntime==1.18.1              # CPU 版本",
            "echo.",
            "echo [模型依赖]",
            "echo   pip install pix2tex==0.1.4                   # pix2tex 模型 (CPU/GPU)",
            "echo   pip install pix2text==1.1.4                  # pix2text 模型 (需 GPU)",
            "echo.",
            "echo [权限问题修复 - 遇到 WinError 5 拒绝访问时使用]",
            "echo   pip install 包名 --upgrade --user            # 安装到用户目录",
            "echo   pip install 包名 --upgrade --force-reinstall # 强制重装",
            "echo.",
            "echo [诊断命令]",
            ...
            "echo   nvidia-smi                                   # 查看 GPU 状态 (驱动支持的最高CUDA版本)",
            "echo   nvcc --version                               # 查看实际安装的 CUDA Toolkit 版本",
            "echo.",
            "echo [清理缓存]",
            "echo   pip cache purge                              # 清理 pip 缓存",
            "echo.",
            "echo ================================================================================",
            "echo.",
        ]
        help_str = " && ".join(help_cmds)
...
cd /d "{venv_dir}"
echo.
echo [!] 管理员模式 - 请谨慎操作，错误命令可能损坏环境！
echo.
REM 设置 PATH，确保使用指定的 Python 环境
set "PATH={pyexe_dir};{scripts_dir};%PATH%"
echo [*] Python 环境: {pyexe_dir}
echo [*] pip/python 命令将使用此环境
echo.
echo ================================================================================
echo                        LaTeXSnipper 环境终端 - 常用命令
echo ================================================================================
...
echo.
echo [ONNX Runtime]
echo   pip install onnxruntime-gpu==1.18.1          # CUDA 11.8
echo   pip install onnxruntime==1.18.1              # CPU 版本
echo.
echo [模型依赖]
echo   pip install pix2tex==0.1.4                   # pix2tex 模型
echo   pip install pix2text==1.1.4                  # pix2text 模型 (需GPU)
echo.
echo [诊断命令]
echo   pip list                                     # 查看已安装包
echo   pip check                                    # 检查依赖冲突
echo   where python                                 # 确认使用的 Python 路径
echo   where pip                                    # 确认使用的 pip 路径
echo   nvcc --version                               # 查看 CUDA Toolkit 版本
...
'''
                ...
                self._show_info("终端已打开", "已请求管理员权限，请在 UAC 弹窗中确认。", "success")
            else:
                # 普通模式 - 也使用批处理文件以确保环境正确激活
                import tempfile
                batch_content_normal = f'''@echo off
chcp 65001 >nul
cd /d "{venv_dir}"
echo.
echo [*] 普通模式 - 如遇权限问题请使用管理员模式
echo.
REM 设置 PATH，确保使用指定的 Python 环境
set "PATH={pyexe_dir};{scripts_dir};%PATH%"
echo [*] Python 环境: {pyexe_dir}
...
'''
                ...
                self._show_info("终端已打开", "请在新窗口中操作，常用命令已显示，修复后重启程序。", "success")
        except Exception as e:
            self._show_info("打开失败", str(e), "error")
    ...
    def _restart_with_wizard(self):
        ...
        try:
            ...
            if script_path.endswith('.py'):
                subprocess.Popen([python_exe, script_path], env=env, creationflags=subprocess.CREATE_NEW_PROCESS_GROUP)
            else:
                # 打包后的 exe
                subprocess.Popen([script_path], env=env, creationflags=subprocess.CREATE_NEW_PROCESS_GROUP)
...
class MainWindow(_QMainWindow):
    ...
    def _add_editor_to_fav(self):
        ...
        if not text:
            self.set_action_status("编辑器为空")
            return
        self.favorites_window.add_favorite(text)
    ...
    def _latex_to_svg_code(self, latex: str) -> str:
        ...
        try:
            ...
            self.set_action_status(f"SVG 转换失败: {e}")
            return ""
    ...
    def _latex_to_mathml(self, latex: str) -> str:
        ...
        try:
            ...
            self.set_action_status(f"MathML 转换失败: {str(e)[:50]}")
            escaped = latex.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            return _mathml_standardize(f'''<math xmlns="http://www.w3.org/1998/Math/MathML">
  <mrow>
    <mtext>{escaped}</mtext>
    ...
    def _latex_to_omml(self, latex: str) -> str:
        ...
        try:
            ...
            self.set_action_status(f"OMML 转换失败: {e}")
            return ""
...
<pre style="background: #fff3e0; padding: 10px; border-radius: 4px; overflow-x: auto;">{str(e)}</pre>
    ...
    def _render_content_block(self, content: str, label: str, content_type: str) -> str:
        ...
        try:
            ...
            # 根据类型渲染内容
            if content_type in ("pix2tex", "pix2text"):
                # 公式模式：使用 MathJax 渲染（专业数学公式渲染，支持完整 LaTeX）
                # 注意：不能对 LaTeX 内容进行 HTML 转义，否则会破坏 LaTeX 语法
    ...
    def _do_fav_row(self, row):
        ...
        if not txt:
            self.set_action_status("内容不存在")
            return
        self.favorites_window.add_favorite(txt)
    ...
    def show_confirm_dialog(self, latex_code: str):
        ...
        # 公式模式：使用 MathJax 渲染
        if current_mode in ("pix2tex", "pix2text"):
...
def show_dependency_wizard(always_show_ui=True):
    ...
    try:
        ...
        if not ok:
            QMessageBox.critical(None, "修复失败", "依赖修复未成功，你这辈子有了。")
        else:
