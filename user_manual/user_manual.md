...
系统托盘菜单还提供 **截图屏幕模式**：自动模式会按鼠标释放点选择屏幕，也可以固定到某一块显示器。多屏截图位置不对时，优先检查这里。
...
| 安装包 | Inno 安装包 / Store MSIX | Debian/Ubuntu `.deb` | `.dmg` / `.app.zip` |

当前快捷键设置入口只接受 `Ctrl+字母` 或 `Ctrl+Shift+字母`，因此默认快捷键和用户可设置快捷键都落在三端共同支持范围内。若 Linux Wayland 环境下快捷键或截图无响应，通常是桌面环境权限或全局快捷键策略限制，优先参考下方 Wayland 说明。
