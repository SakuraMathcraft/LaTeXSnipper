...
- **外部模型：** 连接 OpenAI-compatible、Ollama 或 MinerU Local 服务。推荐预设包括 GLM-OCR、PaddleOCR-VL、Qwen2.5/Qwen3-VL 和 MinerU Local。

外部模型的 **提示词模板** 同时决定请求内容和结果类型。PDF 入口会单独询问导出格式和 DPI。填写 **自定义提示词** 后，自定义提示词优先级最高，会覆盖图片、截图、手写以及 OpenAI-compatible / Ollama PDF 识别的默认模板；MinerU Local 不使用提示词。
...
- 扫描件：尝试 200-300 DPI
- 外部模型普通图片/手写识别：确认设置中的提示词模板与任务匹配
...
**原因：** 提示词模板决定普通图片、截图和手写结果类型；PDF 入口的格式选择独立。自定义提示词会覆盖 OpenAI-compatible / Ollama 的内置模板，包括 PDF 识别；MinerU Local 走文档解析接口，不使用提示词。
