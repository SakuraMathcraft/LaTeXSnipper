...
- #text(weight: "bold")[外部模型：] 连接 OpenAI-compatible、Ollama 或 MinerU Local 服务。推荐预设包括 GLM-OCR、PaddleOCR-VL、Qwen2.5/Qwen3-VL 和 MinerU Local。

外部模型的 #text(weight: "bold")[提示词模板] 同时决定请求内容和结果类型。PDF 入口会单独询问导出格式和 DPI。填写 #text(weight: "bold")[自定义提示词] 后，自定义提示词优先级最高，会覆盖图片、截图、手写以及 OpenAI-compatible / Ollama PDF 识别的默认模板；MinerU Local 不使用提示词。
