...
通过功能区"行内公式""行间公式""带编号公式"按钮或"加载所选"打开的独立公式编辑器窗口：
...
#block(
  ...
  - *数学编辑区：* 全尺寸 MathLive 可视化编辑器，支持键盘快捷键、数学命令和符号面板输入
  - *LaTeX 源码区：* 编辑区下方的 LaTeX 源码输入区，与可视化编辑器实时双向同步
  - *符号面板（右侧）：* 提供 16 类符号库：希腊字母、结构模板、分隔符、微积分、线性代数、关系符、运算符号、大型运算符、箭头、集合、函数、概率统计、化学、物理、字体重音、其他。结构模板支持动态设置矩阵行列数
  - *底部工具栏：* 基础运算（计算、化简、数值化、求解、展开、因式分解），排版（多行布局选择），进阶运算，快捷插入片段，示例载入，复制（LaTeX / MathJSON）
  - *插入 / 更新：* 根据当前模式显示"插入"或"更新"
  - *取消：* 关闭编辑器，不应用更改
]

#tip-block("编辑器快捷键", [
  - `Enter`：在公式编辑器中提交公式（确认插入/更新）
  - `Esc`：关闭编辑器，不应用更改
])
