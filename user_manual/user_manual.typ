...
- *依赖环境：* Windows GitHub 安装包内置规范化依赖环境，Store 包内置 CPU 运行时和模型；Linux 运行时在 `~/.latexsnipper/deps/python311` 创建依赖环境，macOS 在 `~/Library/Application Support/LaTeXSnipper/deps/python311` 创建依赖环境，需要系统 Python 3.10+。
