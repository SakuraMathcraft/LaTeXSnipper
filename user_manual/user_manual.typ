...
选中由 LaTeXSnipper 插入的公式本体或其所在段落后：
...
#block(
  ...
  - *重编号全部：* 按文档顺序重新排列所有自动编号公式的编号，更新元数据。不重新渲染公式内容，仅调整编号。括号样式（圆括号/方括号/花括号/无）可在设置中自定义
...
#block(
  ...
  - *转为 OLE：* 将选中的 OMML 管理公式转为 OLE 公式对象（支持更多渲染选项）；如果选中的是非 LaTeXSnipper 托管的 Word 原生公式，插件会读取原生 OMML，转换为 MathML，并把 MathML 作为源码存入 OLE 元数据
  - *转为 Word：* 将 OLE 公式转为 Word 原生 OMML 公式（可像 Word 公式一样直接编辑）
  - *Word 原生 OMML 转 OLE 边界：* 这类公式可加载和继续转换，但不应执行"格式化所选"，因为格式化链路按 LaTeX 宏重写源码，MathML 源码会被破坏
]
...
下面的复杂度用于估算长文档中的响应时间。设 `n` 为参与操作的公式数，`m` 为文档中 LaTeXSnipper 管理公式总数，`c` 为文档中的内容控件、图片/OLE 对象或形状数量，`f` 为 Word 域数量，`s` 为单个公式源码或 OMML/MathML XML 的长度，`r` 为一次 MathJax 渲染或图片/OLE 生成成本。
...
#table(
  ...
  [删除所选], [`O(c + f + n log n)`], [`O(c + n)`], [定位并按文档位置删除公式、编号公式所在段落、边界控件、引用域或引用占位符],
  [添加编号], [`O(s + r + replace + c + m log m)`], [不适用], [将普通行间公式改为编号布局，随后重编号自动编号公式],
  [重编号全部], [`O(c + m log m + refs)`], [不适用], [收集并排序全部自动编号公式和章/节边界，更新 SEQ 域、编号书签和引用域],
...
#block(
  ...
  Word 原生 OMath 转 OLE 保存的是 MathML 源码，不是 LaTeX 源码；它用于迁移和保真转换，不进入 LaTeX 格式化链路。原生行内公式由 Word 自己维护外层公式输入框，少数单选场景下外层占位框可能由 Word 保留；继续转为 LaTeXSnipper 管理的 OMML 后会由插件接管并清理为插件格式。
]
