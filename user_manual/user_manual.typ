...
#block(
  ...
  - *符号面板（右侧）：* 内置 **18 分类、1006 个数学符号与公式模板**，覆盖从基础算术到前沿研究的全部符号体系：
    #set par(spacing: 0.3em)
    + *基础（4 类 160 条）：* 希腊字母（含变体和希伯来字母）、结构模板（分数/矩阵/根式等，支持自选矩阵行列数）、分隔符（含大尺寸变体和装饰框线）、集合与逻辑符号
    + *代数与几何（3 类 234 条）：* 线性代数（矩阵运算/特征系统）、抽象代数（群环域/同调代数/李理论）、几何（微分几何/黎曼曲率/陈类/Chern-Simons/Atiyah-Singer 指标）
    + *分析（1 类 96 条）：* 微积分/傅里叶分析/积分变换/偏微分方程/泛函分析/索伯列夫空间/特殊函数
    + *拓扑（1 类 82 条）：* 一般拓扑/同调论/同伦论/示性类/莫尔斯理论/谱序列/K 理论
    + *关系与运算（3 类 198 条）：* 比较关系（含各种否定变体）/二元运算（含环论算子）/大型运算符（多重积分/求和乘积）
    + *函数（1 类 56 条）：* 三角函数/反三角函数/双曲函数/对数/极值/实虚部/误差函数/积分正弦余弦
    + *概率统计（1 类 46 条）：* 常见分布/期望方差/条件概率/信息论/极限定理/不等式
    + *物理（1 类 74 条）：* 力学/电磁学/热力学/量子力学/相对论/标准模型/量子场论
    + *化学（1 类 60 条）：* 分子式/化学反应/物化常数/热力学/电化学/光谱学
    + *其他（2 类 78 条）：* 字体重音（mathbf/mathbb/mathcal/mathscr/boldsymbol 等）、杂项符号
  - *符号搜索：* 每个符号面板顶部设有搜索框，支持按中文名称或 LaTeX 源码实时过滤定位符号
