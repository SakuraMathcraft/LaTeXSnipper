...
| 模块 | 已覆盖 | 尚未覆盖 |
|---|---|---|
| Formula domain | trim、换行、mode fallback、空/过长、跨端 metadata 字段 | 文档级 ID 冲突与 schema migration |
...
| 浏览器模式 | 通过 | Office 初始化超时后显示“浏览器预览模式”。 |
...
本阶段所有新增运行时代码都位于 `office_addin/word-macos/`，没有修改桌面 App、Bridge 或 Windows Office 插件。

| 范围 | 状态 | 结果 |
|---|---|---|
| Office Bridge/结构/文档三件套 | 未测试 | 本机只有 Python 3.9.6，低于项目要求的 3.10+，且没有 pytest/PyQt6/cv2。 |
