...
| fixed | User-facing mojibake / dependency log boxes | `src/bootstrap/*`, `src/backend/cuda_runtime_policy.py`, `src/ui/settings_mathcraft_mixin.py` | Dependency install logs and CUDA/cuDNN diagnostics could decode subprocess output with the platform default encoding. Earlier source-file mojibake reports were verified as console display issues by reading source as UTF-8. | Dependency installation, Pandoc checks, package verification, CUDA version detection, and local device probes could show garbled text in logs or diagnostic text. | Subprocess log paths now set `PYTHONUTF8=1`, `PYTHONIOENCODING=utf-8`, `encoding="utf-8"`, and `errors="replace"` where output is captured. Log-style status lines keep ASCII prefixes such as `[OK]`, `[WARN]`, `[ERR]`, and `[HINT]`; visible dependency-wizard status labels may use normal UI emoji/icons. Verified with `python -m compileall -q` on the touched modules and a bootstrap emoji scan. |
...
| fixed | Uninstall data cleanup | `Inno/latexsnipper.iss`, `scripts/latexsnipper-clean-user-data.sh`, `scripts/build_deb.sh`, `scripts/build_macos.sh`, `packaging/debian/DEBIAN/postrm` | Uninstall removed installed files but did not offer a clear path to delete app-managed state, dependency environments, temp files, logs, or MathCraft model weights. | Users who expected a clean uninstall could leave large model caches and dependency environments behind. | Windows uninstaller now prompts before uninstall starts with separate app-data, recorded dependency-root cleanup, and model-weight cleanup checkboxes, and closes `LaTeXSnipper.exe` before file removal. Dependency cleanup reads the active `install_base_dir` plus `install_base_dir_cleanup_roots`, then removes only Python dependency environments from recorded roots and app-managed shared tools from `<app-state>/tools`. It also removes the installed `_internal` directory as install payload. Linux and macOS packages ship a current-user cleanup script with `--deps` because package uninstall must not implicitly delete arbitrary home-directory data. Custom `MATHCRAFT_HOME` directories are intentionally not deleted automatically. |
