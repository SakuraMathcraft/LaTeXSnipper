...
| fixed | Uninstall data cleanup | `Inno/latexsnipper.iss`, `scripts/latexsnipper-clean-user-data.sh`, `scripts/build_deb.sh`, `scripts/build_macos.sh`, `packaging/debian/DEBIAN/postrm` | Uninstall removed installed files but did not offer a clear path to delete app-managed state, dependency environments, temp files, logs, or MathCraft model weights. | Users who expected a clean uninstall could leave large model caches and dependency environments behind. | Windows uninstaller now prompts before uninstall starts with separate app-data, recorded dependency-root cleanup, and model-weight cleanup checkboxes. Dependency cleanup reads the active `install_base_dir` plus `install_base_dir_cleanup_roots`, then removes only known managed children such as `python311`, `pandoc`, and `translation_env`. It also removes the installed `_internal` directory as install payload. Linux and macOS packages ship a current-user cleanup script with `--deps` because package uninstall must not implicitly delete arbitrary home-directory data. Custom `MATHCRAFT_HOME` directories are intentionally not deleted automatically. |
