...
所有截图、图片、PDF、手写识别、数学计算，全部在本地处理，不上传云端。只有在你自己选择使用外部 AI 模型时，数据才会发送到你指定的服务。
...
| 功能 | 用法场景 |
|------|---------|
| **插入公式** | 行内公式、行间公式、带编号公式——LaTeX 写好，插件可渲染为 OLE 对象或 Word OMML |
| **截图识别** | 在 Word 里写论文时，看到其他资料的公式，截个图直接把公式插入进来 |
| **加载/修改/删除** | 已插入的公式可以重新加载到编辑器中修改，改完再更新回去，编号不乱 |
| **自动编号** | 给行间公式加编号，可以选左编号或右编号，并支持章/节层级前缀 |
| **全篇重编号** | 调整了章节顺序？一键更新自动编号公式的 SEQ 字段、编号书签和交叉引用 |
| **章/节分隔符** | 写长篇论文时，公式编号自动带上章号（如 (1.1)、(1.2)），层级分隔符可选 `.` `-` `·` `:` `/` |
| **交叉引用** | 在正文中插入公式编号引用，选中目标公式或其所在段落即可生成，Word 原生支持刷新 |
| **公式转换** | OLE 对象和 Word OMML 格式之间互转，也可把选中的 Word 原生 OMML 公式转为插件 OLE |
| **格式重置** | 公式字体乱了、颜色不对？一键恢复默认 |
| **字体/颜色设置** | 设定新插入公式和“格式化所选”的默认字体（TeX 原生、罗马、粗体、斜体、黑板粗体、花体、哥特体、无衬线、等宽等）和字体颜色 |
