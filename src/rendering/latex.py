...
VALID_LATEX_RENDER_MODES = frozenset(
    {"auto", "mathjax_local", "mathjax_cdn", "latex_pdflatex", "latex_xelatex"}
)


class LaTeXConfigStore(Protocol):
...
def get_document_render_mode() -> str:
    if _latex_config is None:
        return DEFAULT_LATEX_RENDER_MODE
    mode = str(_latex_config.get(LATEX_RENDER_MODE_KEY, DEFAULT_LATEX_RENDER_MODE) or "").strip()
    return mode if mode in VALID_LATEX_RENDER_MODES else DEFAULT_LATEX_RENDER_MODE


def get_latex_executable_path() -> str:
    if _latex_config is None:
        return ""
    return str(_latex_config.get(LATEX_EXECUTABLE_PATH_KEY, "") or "").strip()
...
_latex_config: LaTeXConfigStore | None = None


def configure_latex_settings(config: LaTeXConfigStore) -> None:
    """Bind LaTeX rendering preferences to the application config store."""
    global _latex_config, _latex_renderer
    _latex_config = config
    _latex_renderer = None


def update_latex_settings(
    *,
    render_mode: str | None = None,
    executable_path: str | None = None,
) -> None:
    """Persist LaTeX preferences through the shared application config."""
    global _latex_renderer
    if _latex_config is None:
        raise RuntimeError("LaTeX 配置尚未初始化")

    values: dict[str, object] = {}
    if render_mode is not None:
        normalized_mode = str(render_mode or "").strip()
        if normalized_mode not in VALID_LATEX_RENDER_MODES:
            raise ValueError(f"无效的公式渲染模式: {render_mode}")
        values[LATEX_RENDER_MODE_KEY] = normalized_mode
    if executable_path is not None:
        values[LATEX_EXECUTABLE_PATH_KEY] = str(executable_path or "").strip()
    changed = {
        key: value
        for key, value in values.items()
        if _latex_config.get(key, None) != value
    }
    if LATEX_EXECUTABLE_PATH_KEY in changed:
        _latex_renderer = None
    if changed:
        _latex_config.set_many(changed)
...
def get_latex_renderer() -> LaTeXRenderer:
    ...
    if _latex_renderer is None:
        _latex_renderer = LaTeXRenderer(get_latex_executable_path() or None)
