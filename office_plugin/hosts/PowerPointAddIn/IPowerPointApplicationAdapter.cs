...
public interface IPowerPointApplicationAdapter
{
    ...
    Task InsertOleFormulaObjectAsync(FormulaMetadata metadata, CancellationToken cancellationToken);

    Task InsertOleFormulaObjectAtPositionAsync(FormulaMetadata metadata, float left, float top, CancellationToken cancellationToken);
